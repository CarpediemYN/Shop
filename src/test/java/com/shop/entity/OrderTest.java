...
//				order.getOrderItems().add(orderItem); //아직 영속성 컨텍스트에 저장되지 않은 OrderItem 엔티티를 Order엔티티에 저장
//			}
//		
//		
...
//		order.getOrderItems().remove(0);	// Order 엔터티에서 관리하고 있는 OrderItem 리스트의 0번째 인덱스 요소를 제거한다.
//		em.flush();
//	}
	...
	public void lazyLoadingTest() {
		...
		em.flush();
		em.clear();
		OrderItem orderItem = orderItemRepository.findById(orderItemId)	// Order엔티티에 저장했던 주문 상품 아이디를 이용하여 OrderItem을 DB에서 조회한다.
				.orElseThrow(EntityNotFoundException::new);
